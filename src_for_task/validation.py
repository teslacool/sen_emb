...
class InnerKFoldClassifier(object):
    ...
    """
    def __init__(self, enc_input_train,enc_input_valid, label, config):
        self.enc_input_train = enc_input_train
        self.enc_input_valid = enc_input_valid
        self.label = label
        self.featdim = enc_input_train.shape[1]
    ...
    def run(self):
        ...
        innerskf = StratifiedKFold(n_splits=self.k, shuffle=True,
                                   random_state=1111)
        count = 0
        for train_idx, test_idx in skf.split(self.enc_input_train, self.label):
            count += 1
            X_train, X_test = self.enc_input_train[train_idx], self.enc_input_train[test_idx]
            valid_train, _  = self.enc_input_valid[train_idx], self.enc_input_valid[test_idx]
            y_train, y_test = self.label[train_idx], self.label[test_idx]
            scores = []
            for reg in regs:
                regscores = []
                for inner_train_idx, inner_test_idx in innerskf.split(X_train, y_train):
                    X_in_train, X_in_test = X_train[inner_train_idx], valid_train[inner_test_idx]
...
class SplitClassifier(object):
    ...
    def run(self):
        ...
        regs = [10**t for t in range(-5, -1)] if self.usepytorch else \
               [2**t for t in range(-2, 4, 1)]
        # regs = [10 ** t for t in range(-10, -1)] if self.usepytorch else \
        #     [2 ** t for t in range(-2, 4, 1)]
        if self.noreg:
            regs = [0.]
        scores = []
