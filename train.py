    ...
    with torch.no_grad():
        ...
        if top1 > best_top:
            ...
            if params.stage_u==0:
                ...
                # torch.save(model.lstm.state_dict(),save_dir + '/lstm.dict' )
...
def main():
    ...
    if params.val:
        params.reload = True
        val(model,val_data,train_data,params)
        sys.exit("goodbye!")

    if params.stage_u==1:
        optimizer = torch.optim.Adam([
            {'params': model.remove.parameters()},
            # {'params': model.mapping.parameters()},
            # {'params': model.mapping1.parameters()}
        ],lr=0.000002)
    else:
        optimizer = torch.optim.Adam([
    ...
    train(model,train_data,val_data,optimizer, 10000,params)
